...
/* Get the min and max vals from pools collection using id poolName, take a random value between min and max
and try to insert in the collection named poolName, but first findOne on the collection to see if the random value
is already there. If it is and the size of collection poolName is less than max-min, then keep searching. */
